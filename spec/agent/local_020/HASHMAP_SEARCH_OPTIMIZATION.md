...
> **⚠️ アーカイブ済み (2026-01-18)**
>
> この仕様書で提案した `hashmap-search` feature は実装後の評価において、
> 想定していたほどの性能改善が得られなかったため、コードベースの複雑化を
> 避ける目的で削除されました。
>
> 削除コミット: `remove-hashmap-search` ブランチ
>
> 二分探索版（`search_seeds_x16`）が引き続きデフォルトの検索実装として使用されます。
